...
(2) UE5 C++ 会自动忽略未引用的头文件里的各种数据结构。(写的有多错，都不会报错，除非加到一个有逻辑的代码块里)
...
(3) 对: 还是全局Map 索引，但我们在对象中定义一个 64位整数作为静态变量，每分配一次，这个Id就加1，这样，我们拿这个Id 作为外部Handle的索引。 把Id 作为Map的Key, 指针作为Map的Value。(更简单的做法：因为指针本身就是64位整数，所以转为64位整数，作为Map的key就行了。 如果Map 不存在这个Key, 说明这个指针就是野指针)
